/*************************************
CS 330 Lab 9: Operator overloading, namespaces, and static members
...
namespace Mem {
  ...
  public:
    ...
    friend std::ostream & operator<<(std::ostream &s, IntQueue &q);
    IntQueue & operator<<(int newValue);
  ...
  // TODO: Add prototypes for any of IntQueue's friends here
    ...
    std::ostream & operator<<(std::ostream &s, IntQueue &q);
}
...
std::ostream & Mem::operator<<(std::ostream &s, Mem::IntQueue &q) 
{
  Mem::Elem<int> *current = q.head;
  while (current != NULL) {
    s << current->getData() << " ";
...
Mem::IntQueue & Mem::IntQueue::operator<<(int newValue) 
{
...
//
// Overload >> operator for dequeuing int
//
