...
void construct_adj_list(int** adj_mat, int rows, int cols, adj_node_t*** list)
{
    ...
    //Row based parsing through the matrix
    for (int i = 0; i < rows; i++)
    {
	    //column based search
	    for (int j = 0; j < cols; j++)
	    {
		//look for edge, create note and add to adj list
		if(adj_mat[i][j] != 0)
		{
         ...
         int* color, int* distance, int* parent)
{
	...
	init_adj_list(&Q, rows);
	assert(Q);

	adj_node_t *src_node = create_node(source);
	add_node(Q, 0, src_node);
	...
	while(items_in_Q != 0)
	{
		//First in first out, dequeue 1st
		int u = remove_node(Q);
		
		//Decrementation of # of items in Q
		items_in_Q--;
		...
		while(v != NULL)
		{
			//if not visited or in queue, add
			if(color[v->vid] == 0)
			{
				color[v->vid] = 1;
