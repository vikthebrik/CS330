...
{
	//memory allocation for the array being produced
	...
	*csr_vals = (double*)malloc(nnz * sizeof(double));
	...
	//create a hist of size m to count the rows in the C00
	unsigned int hist[m + 1];
	for (int i = 0; i < m + 1; i++)//init of above hist
	{
		hist[i] = 0;
	}

	//fill hist with appropriate values(# of rows in COO)
	for (int i = 0; i < nnz; i++) 
	{
		hist[row_ind[i] - 1] += 1;
	}
	
	//Calculate and store index values for csr_row_ptr
	for (int i = 0; i < m + 1; i++) 
	{
		// Start at index 1
		if (i == 0) {
		...
		unsigned int calc = (*csr_row_ptr)[i - 1] + hist[i - 1];
		(*csr_row_ptr)[i] = calc;
	}

	//copy csr_row_ptr
	unsigned int temp[m + 1];
	for (int i = 0; i < m + 1; i ++)
	{
		unsigned int val = (*csr_row_ptr)[i];
		temp[i] = val;
	}

	//fill the csr_col_index array and the csr_vals array
	for (int i = 0; i < nnz; i++) {
		// hold index value
		unsigned int ind = temp[row_ind[i] - 1];

		// Increment index pointer
		temp[row_ind[i] - 1]++;

		// Store the column value at the index in csr_col_ind
		(*csr_col_ind)[ind - 1] = col_ind[i];

		// Store the value at the index in csr_vals
		(*csr_vals)[ind - 1] = val[i];
	}
...
{
	for (int i = 0; i < m; i++) {
		// Obtain the start and end values
		int start = csr_row_ptr[i] - 1;
		int end = csr_row_ptr[i + 1] - 1;

		// Loop through each row and compute the matrix multiplication
