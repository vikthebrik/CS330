                  ...
                  unsigned int remain)
{
    ...
    // insert_node() test
    RBTNode** nodes = new RBTNode*[num_keys];
    RBT my_RBT;
    to << endl << "Insert test: " << endl;
    for(int i = 0; i < num_keys; i++) {
        nodes[i] = new RBTNode(keys[i]);
        my_RBT.rb_insert_node(nodes[i]);
        to << "=====" << " inserting " << nodes[i]->get_key() << " =====" 
             << endl;
        my_RBT.walk(to);
        to << "========================" << endl;
    }
    ...
    to << "Min/max test: " << endl;
    to << "----------" << endl;
    to << "RBT min: "; 
    my_RBT.tree_min()->print_info(to);
    to << endl;
    to << "RBT max: "; 
    my_RBT.tree_max()->print_info(to);
    to << endl;
    to << "----------" << endl;
    to << endl;

    // walk() test
    to << "BST walk test: " << endl;
    to << "----------" << endl;
    my_RBT.walk(to);
    to << "----------" << endl;
    to << endl;
    ...
    to << "----------" << endl;
    to << "Predecessor/Successor test: " << endl;
    for(int i = 0; i < num_keys; i++) {
        
        // to << nodes[i]->get_key() << " ";
        // nodes[i]->print_color(to);
        nodes[i]->print_info(to);
    ...
    for(int i = 0; i < num_keys; i++) {
        RBTNode* tmp = my_RBT.tree_search(keys[i] + 1);
        if(tmp != my_RBT.get_sentinel()) {
    ...
    #if 0
    // Probably not do this since you can't randomly select a node
    // left_rotate test
    to << "----------" << endl;
    to << "Left rotate test" << endl;
    my_RBT.left_rotate(nodes[5]);
    my_RBT.walk(to);
    cout << (nodes[2]->get_left())->get_key() << endl;
    cout << (nodes[2]->get_right())->get_key() << endl;
    cout << (nodes[5]->get_left())->get_key() << endl;
    cout << (nodes[5]->get_right())->get_key() << endl;
    cout << (nodes[0]->get_left())->get_key() << endl;
    cout << (nodes[0]->get_right())->get_key() << endl;

     // right test
    to << "----------" << endl;
    to << "Right rotate test" << endl;
    my_RBT.right_rotate(nodes[2]);
    my_RBT.walk(to);
    cout << (nodes[5]->get_left())->get_key() << endl;
    cout << (nodes[5]->get_right())->get_key() << endl;
    cout << (nodes[2]->get_left())->get_key() << endl;
    ...
    for(unsigned int i = 0; i < num_keys - remain; i++) {
        ...
        RBTNode* tmp = my_RBT.tree_search(keys[i]);
        if(tmp != my_RBT.get_sentinel()) {
            my_RBT.rb_delete_node(tmp);
            my_RBT.walk(to);
        } else {
